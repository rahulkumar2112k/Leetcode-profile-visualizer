    ...
    try {
      ...
      if (res.ok && data?.data?.matchedUser) {
        ...
        const processed = {
          ...
          totalSolved: user.submitStatsGlobal.acSubmissionNum.find(
            (d) => d.difficulty === "All"
          )?.count || 0,
          easySolved: user.submitStatsGlobal.acSubmissionNum.find(
            (d) => d.difficulty === "Easy"
          )?.count || 0,
          mediumSolved: user.submitStatsGlobal.acSubmissionNum.find(
            (d) => d.difficulty === "Medium"
          )?.count || 0,
          hardSolved: user.submitStatsGlobal.acSubmissionNum.find(
            (d) => d.difficulty === "Hard"
          )?.count || 0,
        };
